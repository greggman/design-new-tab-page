...
export default function spectrumRings() {
  ...
  const arc = chance(0.3) ? choice([3, 3]) : rand(55, 175);
  const sat = rand(58, 90), lit = ctx.P.dark ? rand(44, 62) : rand(48, 66);
  for (let k = n; k >= 1; k--) { const t = k / n; circle({ x: cx, y: cy, w: t * maxR * 2, h: t * maxR * 2, color: hslToHex(h0 + dir * t * arc, sat, lit * (k % 2 ? 1 : 0.75)), z: n - k }); }
